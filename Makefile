...
.PHONY: test
test: $(REGISTRY)/kali-devcontainer ## Test runtime container image
	$(DOCKER) run --interactive --tty --rm --name=kali-devcontainer $<
...
clean: ## Remove all container images
	$(DOCKER) stop kali-devcontainer || true
